...
export class CreateInteractionWorkComponent implements OnInit {
  ...
  postRevealedObjectToReplace(originalParagraphId: any, newRevealedObject: any, isShownotHide: boolean) {
    ...
    fetch((environment.apiUrl + "/api/v1/post-revealed-object-to-replace"), {
      method: 'POST',
      mode: 'cors',
      // headers: {
  ...
  publishNewParagraph() {
    ...
    let paragraph = {
      ...
      "revealed": this.makeRevealedFromParagraph(this.resortedInputValues, this.selectedLineIndex)
  ...
  splitRevealedAtIndex(
    ...
    // flip reveal score : hide ->  reveal or refeal -> hide
    const new_revealed_score: 0 | 1 = old_revealed_score === 1 ? 0 : 1;

    // if empty interval or single character change the revealed score and return (just in case)
